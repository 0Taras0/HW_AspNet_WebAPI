...
{
    public class OrderService(IAuthService authService, AppDbContext context, IMapper mapper, ISMTPService smtpService) : IOrderService
    {
        public async Task CreateOrderAsync(DeliveryInfoCreateModel model)
        {
            ...
            if (user != null && user.Carts != null)
            {
                ...
                var price = orderItems.Sum(i => i.Quantity * i.PriceBuy);
